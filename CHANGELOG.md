...
- **`bouten` プラグイン (opt-in)** — 傍点・傍線 (text-emphasis)。Pandoc 風の
  クラス付きスパン `[テキスト]{.sesame_dot}` → `<em class="sesame_dot">テキスト</em>`。
  クラス名 1 個を `<em class>` に透過するだけで、見た目 (どのクラスがゴマ点・
  二重傍線か) は CSS 側が定める。青空文庫の傍点9種・傍線5種などを無損失に
  運べる。`]` の直後が `{.class}` でなければ何もしないのでリンク `[x](y)`・
  素の `[x]` を壊さない。inner はプレーンテキスト扱い (ruby と同じく推測しない)。
  `*`/`**` (強調・太字) は素の Markdown に任せ対象外。構文追加なので
  `cjk_friendly` とは独立の `md.use(bouten)` として提供 (既定の挙動は変わらない)。
